...
export default function ItaReportDashboard() {
  ...
  async function openInvoice(client: Client) {
    ...
    // Fetch all finalized bookings for this client that haven't been paid yet
    ...
    if (error) {
        alert("Database error loading bookings.");
        setLoading(false);
        return;
    }

    // Extract unique dates from the slot_keys (e.g. "2026-02-02|08:30" -> "02/02/2026")
    ...
    setUnitPrice(70); // Default hourly rate for Ita Job
    setCustomItems([]);
    setLoading(false);
  }
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-green-100 overflow-hidden text-[#16202e]">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                        <tr className="border-b border-gray-100 text-xs uppercase tracking-wider text-green-600 bg-green-50/30">
                            ...
                            return (
                                <tr key={client.id} className={`transition-colors border-b border-gray-50 ${isOwing ? 'bg-red-50/30 hover:bg-red-50/80' : 'hover:bg-green-50/30'}`}>
                                    ...
                                    <td className="px-6 py-5">
                                        {isOwing ? (
                                            <span className="text-red-600 font-black text-lg">{unpaid} hrs</span>
                                        ) : (
                                    ...
                                    <td className="px-6 py-5 text-center">
                                        {isOwing && (
                                            <button 
                                                onClick={() => openInvoice(client)}
                                                className="px-4 py-2 bg-white border-2 border-green-500 text-green-600 font-bold rounded-lg shadow-sm hover:bg-green-50 transition-colors text-xs uppercase tracking-wider"
                                            >
                                                📄 Create
                                            </button>
                                        )}
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        {isOwing && (
                                            <button 
                                                onClick={() => markInvoicePaid(client.id, client.name)}
                                                className="px-4 py-2 bg-green-500 text-white font-bold rounded-lg shadow hover:bg-green-600 transition-colors text-xs uppercase tracking-wider"
                                            >
                                                ✅ Mark Paid
                                            </button>
                                        )}
                    ...
                    <div className="flex-1 p-4 md:p-8 bg-gray-100 overflow-y-auto">
                        <div id="printable-invoice" className="bg-white mx-auto shadow-sm p-8 md:p-12 text-[#16202e] text-sm" style={{ width: '100%', maxWidth: '800px', minHeight: '1000px', fontFamily: 'Arial, sans-serif' }}>
                            ...
                            <div className="grid grid-cols-2 gap-y-4 mb-8">
                                ...
                                <div>Landscaping & Handyman Services</div>
